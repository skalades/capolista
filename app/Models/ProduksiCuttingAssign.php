...
class ProduksiCuttingAssign extends Model
{
    use HasFactory;

    protected $table = 'produksi_cutting_assigns';
    ...
    {
        return $this->hasMany(ProduksiCuttingOutput::class, 'assign_id');
    }

    public function tarifHistories(): HasMany
    {
        return $this->hasMany(TarifCuttingHistory::class, 'assign_id');
    }

    public function qcRejects(): HasMany
    {
        return $this->hasMany(ProduksiCuttingQcReject::class, 'assign_id');
    }
    ...
    {
        $approved = (int) $this->outputs()->where('status', ProduksiCuttingOutput::STATUS_APPROVED)->sum('pcs_approved');
    ...
    public function getTotalRincianSelesaiAttribute(): array
    {
        $outputs = $this->outputs()->whereIn('status', [
            ProduksiCuttingOutput::STATUS_MENUNGGU_APPROVAL,
            ProduksiCuttingOutput::STATUS_APPROVED
        ])->get();
    ...
    {
        return (float) $this->outputs()->where('status', ProduksiCuttingOutput::STATUS_APPROVED)->sum('upah_kotor');
    }
}
